...
const records: SearchRecord[] = [
  ...speciesRecords.map((item) => ({ id: item.id, type: "Species · Leaf Ledger", title: item.commonName, subtitle: item.scientificName, keywords: [item.genus, item.catalogueNumber], href: `/ledger/${item.slug}` })),
...
export function UniversalSearch() {
  ...
  const results = useMemo(() => deferred ? records.filter((item) => `${item.title} ${item.subtitle} ${item.keywords.join(" ")}`.toLowerCase().includes(deferred)) : records.slice(0, 7), [deferred]);
